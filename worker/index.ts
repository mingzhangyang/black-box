...
import { buildLocalizedPath, DEFAULT_LANG, detectPreferredLanguage, isLang, LANGUAGES, normalizePath, parseRoute } from '../src/siteLanguage';
import type { Lang, RouteInfo } from '../src/siteLanguage';
import { buildOutputLanguageInstruction, isPersonaId, PERSONA_INSTRUCTIONS } from '../src/personas';
...
async function consumeRateLimit(limiter: RateLimit, key: string): Promise<RateLimitDecision> {
  try {
...
function rateLimitResponse(decision: Exclude<RateLimitDecision, 'allowed'>): Response {
  if (decision === 'unavailable') {
    return Response.json({ error: 'Rate limiting is temporarily unavailable. Please try again later.' }, { status: 503 });
  }
...
async function handleCreateShare(request: Request, env: Env): Promise<Response> {
  ...
  const rateLimitDecision = await consumeRateLimit(env.RATE_LIMITER, `share:${ip}`);
...
async function handleGenerate(request: Request, env: Env): Promise<Response> {
  const ip = getClientIp(request);
  const rateLimitDecision = await consumeRateLimit(env.RATE_LIMITER, `generate:${ip}`);
  if (rateLimitDecision !== 'allowed') {
    return rateLimitResponse(rateLimitDecision);
  }

  let body: { input?: string; personaId?: string; lang?: string };
  try {
    body = await request.json();
  } catch {
    return Response.json({ error: 'Invalid JSON body' }, { status: 400 });
  ...
  const { input, personaId, lang } = body;
  if (!input || !personaId) {
    return Response.json({ error: 'Missing input or personaId' }, { status: 400 });
  }
  if (!isLang(lang)) {
    return Response.json({ error: 'Missing or invalid lang' }, { status: 400 });
  }
  ...
  const persona = PERSONA_INSTRUCTIONS[personaId];

  const instruction = persona.instruction + buildOutputLanguageInstruction(lang, persona.languageMode);
